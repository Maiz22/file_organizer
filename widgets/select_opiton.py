...
class SelectOption(tb.Frame):
    def __init__(self, root, callback, label:str, directory:str="", cancel=True, tip=None) -> None:
        super().__init__(root)
        self.label = label
        self.directory = directory
        self.callback = lambda event, name=self.label, view_element=self: callback(event, name, view_element)
        self.tip = tip
        self.init_ui()
        if cancel:
    ...
    def init_ui(self) -> None:
        ...
        self.edit_button.bind("<Button-1>", self.callback)
