...
class SelectOption(tb.Frame):
    def __init__(
        ...
        self.edit_callback = lambda event, name=self.label: edit_callback(
            event, name
        )  # , view_element , view_element=self
        self.delete_callback = lambda event, name=self.label: delete_callback(
            event, name
        )
        self.tip = tip
        self.init_ui()
    ...
    def add_active_cancel_btn(self) -> None:
        self.cancel_button = tb.Button(
            self, bootstyle="danger-outline", text="x", command=self.destroy
        )
    ...
    def add_inactive_cancel_btn(self) -> None:
        self.cancel_button = tb.Button(
            self, bootstyle="disabled-outline", text="x", state="disabled"
        )
        self.cancel_button.pack(padx=5, pady=2, side="left")
